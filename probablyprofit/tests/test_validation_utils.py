...
from probablyprofit.utils.validation_utils import clean_json_string, validate_and_parse_decision
...
def test_validate_and_parse_embedded_json():
    ...
    json_str = (
        'Here is the response: {"name": "test", "value": 42, "confidence": 0.9} Hope this helps.'
    )
    obj = validate_and_parse_decision(json_str, MockModel)
    assert obj.name == "test"
    assert obj.value == 42
